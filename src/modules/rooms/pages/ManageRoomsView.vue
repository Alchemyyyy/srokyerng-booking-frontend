...
const { loading, error, selectedPropertyId, propertyFilterTabs, filteredRooms } = storeToRefs(roomStore);
...
<template>
  <div class="my-25 space-y-6 min-h-screen text-(--color-text) transition-colors duration-300">

    <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold tracking-tight">Rooms</h1>
        <p class="text-sm text-(--color-muted) mt-1">Manage rooms across all your properties.</p>
      </div>
    ...
    <nav class="flex flex-wrap gap-3 py-2">
      <AppButton v-for="tab in propertyFilterTabs" :key="tab.id" @click="setSelectedPropertyId(tab.id)" :class="[
        ...
        <span :class="[
          ...
          selectedPropertyId === tab.id ? 'bg-(--color-primary) text-white' : 'bg-(--color-surface-soft) text-(--color-muted)'
        ]">
          {{ tab.roomCount }}
        </span>
      </AppButton>
    ...
    <main class="relative min-h-100">
      ...
      <div v-if="loading" class="absolute inset-0 flex items-center justify-center">
        <div class="animate-spin rounded-full h-10 w-10 border-2 border-t-(--color-primary) border-(--color-border)">
        </div>
      </div>

      <div v-else-if="filteredRooms.length === 0"
        class="text-center py-20 border border-dashed border-(--color-border) rounded-xl">
        <p class="text-sm text-(--color-muted)">No active room templates mapped to this specific asset filter.</p>
      </div>
        ...
        <div v-for="(room, index) in filteredRooms" :key="room.id"
          ...
          <div class="p-5 space-y-4">
            <div class="flex items-start justify-between gap-2">
              <div>
                <h3 class="text-lg font-bold tracking-tight text-(--color-text)">{{ room.type }}</h3>
                <p class="text-xs text-(--color-muted) mt-0.5">Double · {{ room.propertyName }}</p>
              </div>

              <span :class="[
                'px-3 py-1 rounded-full text-xs font-bold tracking-wide border',
                getRoomMeta(room, index).status === 'Available' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : '',
                getRoomMeta(room, index).status === 'Occupied' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : '',
                getRoomMeta(room, index).status === 'Maintenance' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : ''
              ]">
                ● {{ getRoomMeta(room, index).status }}
              </span>
            </div>

            <div class="flex items-center gap-4 text-xs font-medium text-(--color-muted)">
            ...
            <p class="text-sm text-(--color-muted) line-clamp-2 pt-1 font-normal leading-relaxed">
          ...
          <div
            ...
            <button @click="alert(`Delete system prompt triggered for instance context: ${room.id}`)"
