...
const normalizeReservationItem = (item) => ({
    ...
    // ប្រាកដថា Status ទាំងអស់ត្រូវបានបម្លែងទៅជាអក្សរតូចដើម្បីងាយស្រួលផ្ទៀងផ្ទាត់
...
<template>
    <main class="owner-payment-details space-y-6 my-25" :class="isSidebarOpen ? 'ml-64' : 'ml-20'">
        ...
        <div v-else-if="reservation" class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-7 space-y-6 bg-(--color-surface) rounded-2xl border border-(--color-border) p-6 shadow-sm">

                <div class="space-y-3">
                    <span class="text-xs font-bold text-(--color-muted) uppercase tracking-wider block border-b border-(--color-border) pb-1.5">Guest Information</span>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-(--color-surface-soft) p-4 rounded-xl border border-(--color-border)">
                        <div>
                        ...
                        <div>
                            ...
                            <span class="font-semibold text-(--color-primary) text-sm">{{ reservation.guestPhone }}</span>
                        </div>
                        <div class="overflow-hidden">
                            <span class="text-[10px] text-(--color-muted) uppercase font-bold block">Email Address</span>
                            <span class="font-medium text-(--color-text) text-sm truncate block" :title="reservation.guestEmail">{{ reservation.guestEmail }}</span>
                        </div>
                    </div>
                </div>

                <div class="space-y-3">
                    <span class="text-xs font-bold text-(--color-muted) uppercase tracking-wider block border-b border-(--color-border) pb-1.5">Stay Details</span>
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-(--color-surface-soft) p-4 rounded-xl border border-(--color-border)">
                        <div class="col-span-2">
                            <span class="text-[10px] text-(--color-muted) uppercase font-bold block">Property Name</span>
                            <span class="font-bold text-(--color-text) text-sm truncate block">{{ reservation.propertyName }}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <span class="font-bold text-(--color-primary) text-sm">{{ reservation.totalNights }} Night(s)</span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-[10px] text-(--color-muted) uppercase font-bold block">Check-in Date</span>
                            <span class="font-medium text-(--color-text) text-xs">{{ formatDate(reservation.checkIn) }}</span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-[10px] text-(--color-muted) uppercase font-bold block">Check-out Date</span>
                            <span class="font-medium text-(--color-text) text-xs">{{ formatDate(reservation.checkOut) }}</span>
                        </div>
                    </div>
                </div>

                <div class="space-y-3">
                    <span class="text-xs font-bold text-(--color-muted) uppercase tracking-wider block border-b border-(--color-border) pb-1.5">Financial Audit</span>
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-(--color-surface-soft) p-4 rounded-xl border border-(--color-border)">
                        <div>
                            ...
                            <span class="font-black text-(--color-success) text-lg">
                                {{ reservation.currency === 'USD' ? '$' : '' }}{{ reservation.amount.toFixed(2) }} {{ reservation.currency !== 'USD' ? reservation.currency : '' }}
                            </span>
                        </div>
                        <div>
                            <span class="text-[10px] text-(--color-muted) uppercase font-bold block">Payment Method</span>
                            <span class="font-bold text-(--color-text) text-sm mt-1 block">{{ reservation.paymentMethod }}</span>
                        </div>
                        <div class="col-span-2">
                        ...
                        <div class="col-span-2">
                            <span class="text-[10px] text-(--color-muted) uppercase font-bold block">Transaction Reference</span>
                            <span class="font-mono text-xs text-(--color-text) bg-(--color-surface) px-2 py-1 rounded border border-(--color-border) mt-1 block truncate select-all">
                                {{ reservation.txRef || 'N/A' }}
                            </span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-[10px] text-(--color-muted) uppercase font-bold block">Created At</span>
                            <span class="font-medium text-(--color-muted) text-xs">{{ formatDate(reservation.createdAt) }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-5 space-y-3 bg-(--color-surface) rounded-2xl border border-(--color-border) p-6 shadow-sm h-full flex flex-col">
                <span class="text-xs font-bold text-(--color-muted) uppercase tracking-wider block border-b border-(--color-border) pb-1.5">Customer Submitted Proof</span>

                <div class="border border-(--color-border) rounded-xl p-3 bg-white flex justify-center items-center flex-grow min-h-[360px] shadow-inner relative group overflow-hidden">
                    <img v-if="reservation.payment_proof_url" :src="reservation.payment_proof_url" alt="Payment Proof"
                        class="max-w-full h-auto max-h-[420px] object-contain rounded-lg shadow-sm cursor-pointer transition transform hover:scale-105 duration-300"
                        @click="isImageViewerOpen = true" title="Click to view full image" />

                    <div v-else class="text-center p-8">
                        <img :src="defaultReceipt" alt="Placeholder graphic" class="h-full w-full object-contain opacity-70" />
                    </div>

                    <div v-if="reservation.payment_proof_url"
                        class="absolute inset-0 bg-(--color-surface-strong)/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 cursor-pointer pointer-events-none rounded-xl">
                        <span class="text-white text-xs font-bold bg-(--color-primary) px-3 py-1.5 rounded-full shadow">🔍 Click to Enlarge</span>
                    </div>
                </div>

                <div v-if="isImageViewerOpen && reservation.payment_proof_url" class="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4">
                    ...
                    <img :src="reservation.payment_proof_url" alt="Full Payment Proof" class="max-w-full max-h-[90vh] object-contain rounded-xl shadow-2xl" />
                </div>
            </div>

        </div>

        <section v-if="reservation" class="mt-6 flex justify-end gap-3 bg-(--color-surface) border border-(--color-border) p-4 rounded-2xl shadow-sm">
            ...
            <template v-if="reservation.status === 'submitted'">
                ...
                <template v-else>
                    <button type="button" @click="RejectInput = true" :disabled="actionLoading"
                        class="px-5 py-2.5 border border-rose-500/50 text-rose-600 hover:bg-rose-500/10 font-bold rounded-xl text-xs cursor-pointer transition">Reject Payment</button>
