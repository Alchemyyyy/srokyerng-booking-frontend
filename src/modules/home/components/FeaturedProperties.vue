...
const featuredProperties = computed(() =>
  propertyStore.approvedProperties.slice(0, 3).map((property) => ({
    id: property.id,
    name: property.name,
    location: property.city || property.description || t("home.featured.subtitle"),
    price: property.price,
    rating: property.rating,
    reviews: property.reviews,
    category: property.type,
    badge: property.rating >= 4.8
      ? t("home.featured.badges.topRated")
      : t("home.featured.badges.mostPopular"),
    amenities: property.raw?.amenities?.length
      ? property.raw.amenities.slice(0, 2).map((amenity) => amenity.amenity_name || amenity.name || amenity)
      : [t("home.featured.amenities.freeWifi"), t("home.featured.amenities.swimmingPool")],
...
<template>
  <section class="bg-(--color-surface) py-16 px-4 font-sans">
    <div class="mx-auto max-w-7xl">
      <div class="mb-8 flex flex-col justify-between md:flex-row md:items-end">
        <div>
          <div class="mb-2 flex items-center gap-2">
            ...
            <p class="text-[10px] font-bold uppercase tracking-widest text-(--color-primary)">
      ...
      <div v-if="propertyStore.loading" class="rounded-2xl border border-(--color-border) bg-(--color-surface-soft) px-6 py-12 text-center text-(--color-muted)">
        Loading featured properties...
      </div>

      <div v-else-if="propertyStore.error" class="rounded-2xl border border-rose-500/20 bg-rose-500/10 px-6 py-12 text-center text-rose-600">
        {{ propertyStore.error }}
      </div>
        ...
        >
          <div class="relative h-60 overflow-hidden">
            ...
            <div class="absolute left-3 top-3">
              <span class="rounded-full bg-(--color-primary) px-3 py-1 text-[10px] font-bold text-white shadow-lg">
                {{ property.badge }}
              </span>
            </div>
            <div class="absolute bottom-3 left-3 rounded-md bg-(--color-primary)/80 px-3 py-1 text-[10px] text-white backdrop-blur-sm">
              {{ property.category }}
            </div>
          </div>

          <div class="p-5">
            <h3 class="mb-1 text-lg font-bold leading-tight text-(--color-text)">
              {{ property.name }}
            </h3>
            <p class="mb-4 flex items-center gap-1 text-xs text-(--color-muted)">
            ...
            <div class="mb-6 flex flex-wrap gap-2">
              <span
                ...
                :key="amenity"
                class="rounded border border-blue-100 bg-(--color-primary-soft) px-2 py-1 text-[10px] text-(--color-primary)"
              >
                {{ amenity }}
              </span>
            </div>
            ...
            <div class="flex items-center justify-between border-t border-(--color-border) pt-4">
              <div class="flex items-center gap-1">
                ...
                <span class="text-sm font-bold text-(--color-text)">{{ property.rating }}</span>
                <span class="text-xs text-(--color-muted)">({{ property.reviews }})</span>
              </div>
              <div class="text-right">
                <span class="text-lg font-extrabold text-(--color-text)">${{ property.price }}</span>
