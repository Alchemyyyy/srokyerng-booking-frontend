...
export const useAdminDashboardStore = defineStore("admin-dashboard", () => {
    const approvalStore = useApprovalStore(); // ⚡ ភ្ជាប់មកកាន់ Approval Store ដើម្បីយក Real-time Properties Count
    const loading = ref(true);
    const error = ref("");
    const dashboardData = ref(createDefaultDashboardState());

    // 💡 គណនាចំនួន Pending ស្វ័យប្រវត្តិចេញពី Approval Store
    const pendingPropertyCount = computed(() => {
        if (approvalStore.properties.length > 0) {
            return approvalStore.properties.filter(p => Number(p.status_id) === 1).length;
        }
        return dashboardData.value.pending.propertyApprovals;
    });
    ...
    const quickLinks = computed(() => [
        {
            ...
            count: pendingPropertyCount.value, // ⚡ ប្រើ Dynamic count ពី approval store
        },
        {
    ...
    const loadDashboard = async () => {
        ...
        try {
            ...
            // Sync ទាញយក properties មកទុកក្នុង store តែម្ដងដើម្បីឱ្យលេខ count រត់ត្រូវគ្នា
