...
import { formatDate } from '../utils/formatters'; // 💡 ប្រើប្រាស់ Formatter រួមដែលបានបង្កើត
import {
...
// --- States សម្រាប់គ្រប់គ្រង Modals & Actions ---
...
// --- Operations ហៅទៅកាន់ Actions របស់ Store ---
const executeApprove = async () => {
  ...
  if (success) {
    ...
    await approvalStore.fetchPropertyDetail(route.params.id); // ទាញយកទិន្នន័យថ្មីចុងក្រោយ
  } else {
...
const executeSetPending = async () => {
  ...
  if (success) {
    ...
    await approvalStore.fetchPropertyDetail(route.params.id); // គោះយកទិន្នន័យថ្មីចុងក្រោយ
  } else {
...
<template>
  <div class="detail-wrapper transition-all duration-300 my-25" :class="isSidebarOpen ? 'ml-64' : 'ml-20'">
    ...
    <div v-if="previewImageUrl" class="lightbox-layer" @click="closeImagePreview">
      <button class="close-lightbox-btn">&times;</button>
...
.lightbox-layer {
  ...
  width: 100vw;
  height: 100vh;
  background-color: rgba(6, 21, 46, 0.94);
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 9999;
